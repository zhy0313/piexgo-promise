export default class DataFeed {
  ...
  onReady = callBack => new Promise(resolve => {
    resolve();
  }).then(_ => callBack(DataFeed.defaultConfiguration()));
  ...
  }).then(result => onSymbolResolvedCallback(result)).catch(_ => console.log("onResolveErrorCallback"));
  ...
  static defaultSymbol = _ => ({
    name: 'BTC_USDT', // 商品名称。您的用户将看到它(作为一个字符串)。 此外，如果您不使用 tickers ，它将用于数据请求。
    'exchange-traded': '',
    'exchange-listed': '',
    timezone: 'Asia/Shanghai', // 这个商品的交易所时区。我们希望以olsondb格式获取时区的名称
    minmov: 1, // 最小波动
    pointvalue: 1,
  ...
  periodLengthSeconds = (resolution, requiredPeriodsCount) => {
    ...
    if (resolution === '1D') {
      ...
      daysCount = requiredPeriodsCount * resolution / 1440; // (24 * 60)
    }

    return daysCount * 86400; // 24 * 60 * 60
  }
}
